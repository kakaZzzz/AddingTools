...
- (void)recordFetalMovementWithTap:(UITapGestureRecognizer *)tap
{
    ...
    if (!_isRecording) {//第一次点击
        ...
        double seconds1970 = [[NSDate localdate] timeIntervalSince1970];
        
        lastDate = seconds1970;
        //开始记录胎动的代码
        ...
        //在此可以加入时间判断，比如5分钟去重等等
        double seconds1970 = [[NSDate localdate] timeIntervalSince1970];
        if ([self isBeyondFiveMinutes:seconds1970]) {
            for (UIView *label in [aImageView subviews])
            {
                if ([label isKindOfClass:[UILabel class]])
                {
