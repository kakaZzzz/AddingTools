...
typedef enum {
    ADSwitchThumbJustifyLeft,
    ADSwitchThumbJustifyRight
} ADSwitchThumbJustify;

@interface ADSwitchThumb : UIView
@property (nonatomic, assign) BOOL isTracking;
-(void) growThumbWithJustification:(ADSwitchThumbJustify) justification;
-(void) shrinkThumbWithJustification:(ADSwitchThumbJustify) justification;
@end

@interface ADSwitchTrack : UIView
...
@interface ADSwitch () <UIGestureRecognizerDelegate>
@property (nonatomic, strong) ADSwitchTrack* track;
@property (nonatomic, strong) ADSwitchThumb* thumb;
...
-(void) configureSwitch {
    ...
    // Initialization code
    if (!_track) {
        _track = [[ADSwitchTrack alloc] initWithFrame: self.bounds
    ...
    if (!_thumb) {
        _thumb = [[ADSwitchThumb alloc] initWithFrame:CGRectMake(kThumbOffset, kThumbOffset, self.bounds.size.height - 2 * kThumbOffset, self.bounds.size.height - 2 * kThumbOffset)];
...
-(void) setThumbIsTracking:(BOOL)isTracking {
    if (isTracking) {
        //Grow
        [self.thumb growThumbWithJustification: self.isOn ? ADSwitchThumbJustifyRight : ADSwitchThumbJustifyLeft];
    }
    else {
        //Shrink
        [self.thumb shrinkThumbWithJustification: self.isOn ? ADSwitchThumbJustifyRight : ADSwitchThumbJustifyLeft];
    }
...
-(void) setThumbOn:(BOOL) on
    ...
    if (on) {
        thumbFrame.origin.x = self.frame.size.width - (thumbFrame.size.width + kThumbOffset);
        }
    else {
...
-(void) growThumbWithJustification:(ADSwitchThumbJustify) justification {
    ...
    if (justification == ADSwitchThumbJustifyRight) {
