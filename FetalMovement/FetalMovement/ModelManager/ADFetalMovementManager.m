...
- (void)appendData:(NSArray *)datas
{
    ...
    //更新一次本地更新时间
    double seconds = [[NSDate localdate] timeIntervalSince1970];
    [[NSUserDefaults standardUserDefaults] setObject:[NSNumber numberWithDouble:seconds] forKey:LOCAL_UPDATE_TIME];
...
- (NSArray *)getNeedToSyncDataWithStartDate:(double)startTimestamp endDate:(double)endTimestamp;
{
    NSMutableArray *array = [NSMutableArray arrayWithCapacity:1];
    
    NSNumber *star = [NSNumber numberWithDouble:startTimestamp];
    NSNumber *end = [NSNumber numberWithDouble:endTimestamp];
    
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"seconds1970 > %@ AND seconds1970 < %@",star, end];
    
    NSArray *dataArray = [self getDatasFromCoreDataWithPredicate:predicate entityName:@"FetalMovementModel" sortMainKey:@"seconds1970" sortViceKey:nil];
    
    static int year = 0,month = 0,day=0,hour=0;
    // static double hourSeconds = 0;
    
    int count = (int)[dataArray count];
    
    NSLog(@"数组coutn ===== %d",count);
    for (int i =0; i < count; i++) {
        NSMutableString *string = [NSMutableString stringWithCapacity:1];//可变字符串
        
        FetalMovementModel *new = [dataArray objectAtIndex:i];
        year = [new.year intValue];
        month = [new.month intValue];
        day = [new.day intValue];
        hour = [new.hour intValue];
        
        NSDate *date = [NSDate localDateFromString:[NSString stringWithFormat:@"%d.%d.%d %d",year,month,day,hour] withFormat:@"yyyy.MM.dd HH"];
        NSLog(@"原先的日期是%@",date);
        double hourSeconds = [date timeIntervalSince1970];//
        
        [string appendString:[NSString stringWithFormat:@"%d",[new.seconds1970 intValue]]];
        
        for (int j =i + 1; j < count; j ++) {
            
            FetalMovementModel *secondModel = [dataArray objectAtIndex:j];
            year = [new.year intValue];
            month = [new.month intValue];
            day = [new.day intValue];
            hour = [new.hour intValue];
            
            NSDate *nextDate = [NSDate localDateFromString:[NSString stringWithFormat:@"%d.%d.%d %d",year,month,day,hour] withFormat:@"yyyy.MM.dd HH"];
            NSLog(@"下一个原先的日期是%@",date);
            double nextHourSeconds = [nextDate timeIntervalSince1970];//
            if (nextHourSeconds == hourSeconds) {
                [string appendString:[NSString stringWithFormat:@" %d",[secondModel.seconds1970 intValue]]];
                continue;
            }else{
                i = j;
                break;
        ...
        NSLog(@"字符串是,,,,,,%@",string);
        NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithDouble:hourSeconds],@"hour",string,@"data", nil];
        [array addObject:dic];
    }
